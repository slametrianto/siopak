...
class hitungdupak {
	...
	private function Lulus_Unsur_Penunjang_20() {
		...
		//usulan wi
		foreach ($_Ak_dupak_new['id2_subunsur_pak'] as $_id_subunsur => $_nilai_pak) {
			if (in_array($_id_subunsur, $_id_unsur_20))	{
				$_pak_nunjang_jf_20_new += $_nilai_pak;
			}
		}
		...
		// ak yg dipertimbangkan
		$_pertimbangan = $_pak_nunjang_20_prev + $_pak_nunjang_20_new;
		if ($_pertimbangan > $_Ak_diperlukan) { $_pertimbangan = $_Ak_diperlukan; }
		return array('status_lulus'=>$_lulus20, 'ak_diperlukan' => $_Ak_diperlukan,
			...
			'pak_nunjang_jf_20_new'=>$_pak_nunjang_jf_20_new, 'pak_nunjang_20_timbang'=>$_pertimbangan);
	}

	private function Lulus_Unsur_Utama_80() {
		$_pak_utama_80_prev = 0; $_pak_utama_80_new = 0; $_pak_utama_jf_80_new = 0;
		$_pak_ab_80_prev = []; $_pak_ab_80_new = []; $_pak_ab_jf_80_new = [];
		$sql = "select * from master_perhitungan 
		...
		foreach ($_Ak_dupak_new['id2_subunsur_pak'] as $_id_subunsur => $_nilai_pak) {
			if (in_array($_id_subunsur, $_id_unsur_80))	{
				$_pak_utama_jf_80_new += $_nilai_pak;
				$_pak_ab_jf_80_new[$_id_subunsur] += $_nilai_pak;
			}
		}
		...
		return array('status_lulus'=>$_lulus80, 'pak_min' => $_Ak_min_lulus, 
			...
			'pak_ab_jf_80_prev'=>$_pak_utama_jf_80_new, 'pak_ab_jf_80_new'=>$_pak_ab_jf_80_new, 
		);
	}
	...
	private function Lulus_Unsur_Utama_30() {
		...
		foreach ($_Ak_dupak_new['id2_subunsur_pak'] as $_id_subunsur => $_nilai_pak) {
			if (in_array($_id_subunsur, $_id_unsur_30))	{
				$_id_unsur = $_Ak_dupak_new['id2_subunsur_unsur_id'][$_id_subunsur];
				$_pak_utama_jf_30_new += $_nilai_pak;
				$_pak_cd_jf_30_new[$_id_subunsur] += $_nilai_pak;
			}
		}
		...
		return array('status_lulus'=>$_lulus30, 'pak_min' => $_Ak_min_lulus, 
			...
			'pak_utama_jf_30_new'=>$_pak_utama_jf_30_new, 'pak_cd_jf_30_new'=>$_pak_cd_jf_30_new,
		);
	}
	...
	private function Lulus_Jumlah_AK() {		
		...
		$_Ak_dupak_jf_new = $this->Rsakdupaknew['ttl2_pak'] * 1;
		...
		return array('status_lulus'=>$_lulus_jml_ak, 'pak_min' => $_Ak_min_lulus, 
			'pak_lama' => $_Ak_dupak_old, 'ak_diperlukan' => $_Ak_diperlukan,
			'pak_prev'=>$_Ak_dupak_prev, 'pak_new'=>$_Ak_dupak_new, 'pak_jf_new'=>$_Ak_dupak_jf_new);
	}



	private function Lulus_Pengembangan_Profesi() {
		$_pak_utama_pp_prev = 0; $_pak_utama_pp_new = 0; $_pak_utama_jf_pp_new = 0;
		$sql = "select * from master_perhitungan 
		...
		foreach ($_Ak_dupak_new['id2_subunsur_pak'] as $_id_subunsur => $_nilai_pak) {
			if (in_array($_id_subunsur, $_id_unsur_pp))	{
				$_pak_utama_jf_pp_new += $_nilai_pak;
			}
		}
		...
		return array('status_lulus'=>$_luluspp, 'pak_min' => $_Ak_min_lulus, 
			...
			'pak_utama_jf_pp_new'=>$_pak_utama_jf_pp_new);
	}
